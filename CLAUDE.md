...
| `js/studio/studioBalloons.js` | SchreibZauber: Sprechblasen für den Comic-Werktyp (Ausbaustufe 5) - `buildBalloonsHtml()` ist das Comic-Gegenstück zu `buildOverlayHtml()`, plus `addBalloon()`/`updateBalloon()`/`deleteBalloon()`. Der normale Reader kennt Sprechblasen NICHT - beim Export wird der Dialog als normaler Text zusammengefasst (`spreadReadableText()` in `studioExport.js`) |
...
**Bewusste Grenze:** Der normale LeseZauber-Reader kennt Sprechblasen NICHT - das würde die bestehende Trennung "Reader kennt den Unterschied zwischen Werktypen gar nicht" aufweichen. Eine exportierte Comic-Seite zeigt im Reader das Bild plus den Dialog als normalen, vorlesbaren Text ("Name: Zeile", `spreadReadableText()` in `studioExport.js`) - die visuelle Sprechblasen-Darstellung existiert nur in der Werkstatt-Vorschau (Stufe 7). Ein eigener Comic-Druck (mit echten Sprechblasen auf Papier, analog zu `studioPrint.js`) ist noch nicht gebaut - `buildBalloonsHtml()` ist der fertige Baustein dafür. Die in `docs/KONZEPT-Comic.md` Abschnitt 6 beschriebene Panel-Layout-Vorlagen-Bibliothek (mehrere Panels pro Seite) ist ebenfalls noch nicht umgesetzt - eine Comic-"Doppelseite" ist aktuell ein einzelnes, dicht komponiertes Bild pro Seite, kein Panel-Raster.
