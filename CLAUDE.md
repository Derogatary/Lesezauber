...
Seit v0.47.0-beta außerdem **🗺️ Buchatlas** (`js/atlas/`, `docs/BUCHATLAS.md`): eigene Texte (Romane, E-Books) importieren, Wiki (Kapitel, Personen, Orte ...) und Übersetzung erstellen - eigener Bereich mit eigener Bibliothek, wie der SchreibZauber.
...
| `js/backgroundPregen.js` | Opt-in Hintergrund-Vorbereitung, ZWEI unabhängige Schleifen. **Text-Schleife** (`findNextMissingTask()`/`scheduleNext()`), feste Prioritäts-Reihenfolge, alle Stufen teilen sich dasselbe Gemini/Mistral-Tageskontingent: Grundanalyse noch gar nicht ausgelesener Seiten (NEU seit v0.35.1-beta - vorher übersprungen, siehe Wichtiges Verhalten unten) → fehlende Persona-Varianten → Buch-Quiz → Birkenbihl-Übersetzungen (eigener Zusatz-Schalter `backgroundPregenBirkenbihl`). Grundanalyse und Persona-Backfill laufen bei Geschichten über denselben Analyse-Kern `app.actions._analyzePageCore()` (`js/actions/scanner.js`) - ein Aufruf pro Seite reicht für alle fehlenden Personas + Birkenbihl. **Audio-Schleife** (`runOneAudioBackgroundTask()`/`scheduleNextAudio()`, eigener Zusatz-Schalter `backgroundPregenAudio`, NEU seit v0.36.2-beta, FIX v0.36.3-beta): bewusst KOMPLETT GETRENNT von der Text-Schleife, weil sie einen anderen Anbieter mit eigenem Kontingent aufruft (die eingestellte KI-Stimme statt Gemini/Mistral) - eigener Lauf-Schutz (`audioTaskRunning`) statt des gemeinsamen `app.state.apiBusy`, läuft parallel zur Text-Schleife statt dahinter zu warten. `app.ttsNeural.renderAudio()` pro fehlender Seite in den `ttsCache`, nur sichtbar/wirksam mit aktiver KI-Stimme. Pausen-Länge (`audioLoopDelayMs()`, NEU v0.36.4-beta) richtet sich nach dem GERADE eingestellten Anbieter (`provider.bgPregenIntervalMs` in `js/ttsProviders.js`, z.B. Speechify 1200ms laut eigenem Dashboard "1 req/s") statt fest nach den 9 Sekunden der Text-Schleife - ein Anbieter ohne bestätigten Wert fällt auf denselben vorsichtigen Standard zurück |
...
| `js/actions/nightPrep.js`, `js/render/nightPrep.js` | NEU v0.44.0-beta: 🌙 Nachtmodus „Über Nacht vorbereiten“ - App bleibt offen, Screen Wake Lock, schwarze Vollbild-Anzeige `#nightPrepOverlay`, zählt nur zu (`app.utils.countPregenOpen()`), die Arbeit machen weiter die Schleifen in `js/backgroundPregen.js` (die dafür `app.state.pregenActivity` setzen). Seit v0.48.0-beta danach auch vorgemerkte Buchatlas-Aufträge (`runAtlasLoop()`, erst wenn LeseZaubers Text-Aufgaben erledigt sind). Ende bei 0 offen oder 20 min ohne Fortschritt (Tageskontingent), dann Wake Lock frei, schwarz, in installierter App `window.close()`-Versuch. **Echtes Weiterarbeiten bei minimierter PWA ist technisch nicht möglich** (siehe CHANGELOG.md v0.44.0-beta) - nicht erneut versuchen |
...
Aktuell `v0.48.0-beta` (Anzeige im App-Header) - noch nicht veröffentlicht, aktiv in Entwicklung mit einer echten Nutzerfamilie als Testgruppe. Version bei größeren Änderungen hochzählen (Semantic Versioning: `MAJOR.MINOR.PATCH`, `-beta`-Suffix bis zur ersten öffentlichen Veröffentlichung).
