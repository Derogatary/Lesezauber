...
Mit v0.30.0-beta gibt es einen **Hochauflösend-Umschalter** für den Druck (Nutzerfrage nach dem KDP-Innenteil-Export: "wie kommen wir auf die höhere dpi-Zahl?") - `project.spec.highResPrint`, Checkbox "📐 Hochauflösend für den Druck (KDP)" in Stufe 2 ("Der Bauplan"), Standard AUS:
...
Mit v0.29.0-beta gibt es einen echten **KDP-Innenteil-Export** (Nutzerfrage: "wird das nicht von Kindle Create erstellt?", siehe unten die Klarstellung dazu) - `app.studio.printSpreadsKdp()` in `js/studio/studioPrint.js`, eigener Knopf "📕 KDP-Innenteil erzeugen" unterhalb des normalen Druckblocks in Stufe 7:
...
**Korrektur einer bisherigen Annahme:** Der TEIL-F-Nachtrag (siehe unten) hatte als dritten offenen Punkt "Aufteilung jeder Doppelseite in zwei einzelne Trimm-Seiten für die Einreichung" genannt. Das trifft auf diese Architektur tatsächlich NICHT zu: eine "Doppelseite" (`spread`) ist hier von Anfang an bereits EINE physische Druckseite (bei "A5 quer" ein Querformat-Blatt, bei "A5/A4 hoch" ein Hochformat-Blatt) - kein zwei Buchseiten überspannendes Bild. Es gibt also nichts aufzuteilen; dieser Punkt war eine Vorab-Vermutung aus der Recherche, die sich beim tatsächlichen Bau von Ausbaustufe 3 als gegenstandslos herausgestellt hat.

Mit v0.28.0-beta ist der Doppelseiten-Druck (`js/studio/studioPrint.js`) jetzt **comicfähig** - der letzte in v0.27.0-beta bewusst ausgesparte Punkt ("Weiterhin nicht angefasst", siehe dortiger Eintrag):
