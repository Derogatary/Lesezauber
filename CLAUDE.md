...
| `js/backgroundPregen.js` | Opt-in Hintergrund-Vorbereitung. Prioritäts-Reihenfolge: **Grundanalyse noch gar nicht ausgelesener Seiten** (NEU seit v0.35.1-beta - vorher übersprungen, siehe Wichtiges Verhalten unten) → fehlende Persona-Varianten → Buch-Quiz → Birkenbihl-Übersetzungen → **KI-Stimmen-Aufnahmen** (NEU seit v0.36.2-beta, `app.ttsNeural.renderAudio()` pro fehlender Seite in den `ttsCache`, eigener Zusatz-Schalter `backgroundPregenAudio`, nur sichtbar/wirksam mit aktiver KI-Stimme). Grundanalyse und Persona-Backfill laufen bei Geschichten über denselben Analyse-Kern `app.actions._analyzePageCore()` (`js/actions/scanner.js`) - ein Aufruf pro Seite reicht für alle fehlenden Personas + Birkenbihl. Birkenbihl und KI-Stimme sind eigene, zusätzlich zu bestätigende Schalter, da beide über die normalen Analyse-Anfragen hinaus zusätzliche Kosten/Kontingent verbrauchen |
...
Aktuell `v0.36.2-beta` (Anzeige im App-Header) - noch nicht veröffentlicht, aktiv in Entwicklung mit einer echten Nutzerfamilie als Testgruppe. Version bei größeren Änderungen hochzählen (Semantic Versioning: `MAJOR.MINOR.PATCH`, `-beta`-Suffix bis zur ersten öffentlichen Veröffentlichung).
