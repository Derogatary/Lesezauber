...
- **KI-Stimme für die Birkenbihl-Zielsprache** (Bereich "Mehrsprachigkeit"): der 🔊-Knopf im Birkenbihl-Tab liest mit eingestellter KI-Stimme jetzt diese statt der Gerätestimme vor. Neue Route `app.ttsNeural.speakForeign(text, speechLang)` (`js/ttsNeural.js`) - bewusst getrennt von `speak()`: kein deutscher Persona-Stil, keine Audio-Tags, keine deutsche Text-Aufbereitung, keine Hervorhebung. Der `ttsCache` greift weiterhin (Sprache ist Teil des Cache-Schlüssels, bestehende Schlüssel bleiben unverändert gültig) - pro Seite und Sprache also nur beim ersten Antippen ein Synthese-Aufruf. Je Anbieter neues Feld `foreignLanguages` (`js/ttsProviders.js`): Gemini/ElevenLabs/OpenAI sprechen jede Sprache (Gemini und OpenAI bekommen eine englische "mit muttersprachlicher Aussprache vorlesen"-Anweisung statt des deutschen Auftrags), Google Cloud tauscht den Sprachcode im Chirp-3-HD-Stimmennamen aus (`de-DE-Chirp3-HD-Achernar` → `en-GB-Chirp3-HD-Achernar`), Speechify nur Englisch (`simba-3.2`) sowie Französisch/Spanisch/Italienisch/Portugiesisch (`simba-3.0`). Alles andere (Gerätestimme eingestellt, Türkisch/Niederländisch bei Speechify, Fehler) fällt wie bisher auf die Gerätestimme mit passendem Sprachcode zurück - nie ohne Ton. **Noch nicht mit echten Keys getestet** (in der Entwicklungsumgebung keine Anbieter-Keys) - beim ersten Ausprobieren v.a. Speechify mit einer deutschen Stimme auf Englisch/Französisch prüfen.
- **Arbeitsheft-Aufgabentyp "Suchsel 🔍"** (SchreibZauber Stufe 4, bisher als "braucht einen Buchstabengitter-Generator" zurückgestellt): neue Datei `js/studio/wordSearch.js`. Die KI liefert nur die Wörter (`TASK_SCHEMA_HINT` in `js/studio/studioPrompts.js`), das Gitter baut die App selbst - garantiert, dass jedes gelistete Wort wirklich drinsteht (ein Sprachmodell zählt Spalten unzuverlässig). Nur waagerecht/senkrecht (Grundschule), 6×6 bis 10×10, max. 8 Wörter, reproduzierbar über einen Seed (`🔀 Neu mischen` in der Aufgaben-Karte kostet keinen KI-Aufruf). Die Lösung (Zeile/Spalte/Richtung je Wort) erzeugt die App ebenfalls selbst für den Lösungsteil am Heftende. Auf der Heftseite als Kästchengitter mit großen Druckbuchstaben (`js/studio/worksheetCanvas.js`). Damit 6 von 9 Aufgabentypen; offen bleiben nur die drei mit Bildbedarf (Nachspuren, Ausmalen, Schneiden & Kleben).
